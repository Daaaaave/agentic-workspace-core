...
This repository is the upstream source for Agentic Workspace Core. The portable core is project-agnostic; this document explains how this repository uses it while developing the core itself.
...
The universal core should stay free of product-specific facts. Project-specific knowledge belongs in `docs/`; project-specific configuration belongs in `.agents/knowledge.config.json` under `project` when the portable schema has no dedicated field.
