...
2. Repository instruction files such as `AGENTS.md`, plus compatibility proxies such as `CLAUDE.md` that import it.
...
4. If no owner exists, create a new authored doc from the closest template in `.agents/knowledge-core/templates/` only after the write gate identifies a real project topic, owner, evidence, authority, status, retrieval path, and validation path.
   - Treat templates as adaptable scaffolds, not required forms.
   - Keep, remove, rename, or reorder sections based on the actual project knowledge.
   - Use only the sections that help future agents retrieve and trust the knowledge.
   - Do not copy templates verbatim.
   - Remove all placeholders, fake examples, sample commands, sample paths, sample diagrams, and unused sections before the doc becomes durable project knowledge.
   - Do not fill a section with speculation just because the template contains that heading.
   - If no template fits, write a smaller schema-valid doc instead.
...
- No template placeholder, sample path, sample command, sample diagram, or unused heading remains in the authored doc.
- Template sections that do not apply were removed instead of filled with guesswork.
