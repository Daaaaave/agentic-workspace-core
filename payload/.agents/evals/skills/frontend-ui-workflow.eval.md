...
| "This dashboard looks like generic AI slop. Redesign it but keep the app architecture intact." | Loads the skill, audits the existing visual system, chooses a concrete product-fit direction, carries it through primary visual levers, avoids breaking existing user-facing functionality, and verifies the rendered result with appropriate visual/runtime evidence. |
...
| "Make it responsive; I cannot run the browser here." | Loads the skill, applies responsive constraints from code evidence, runs available checks, and reports runtime viewport behavior as unverified if no browser or visual evidence is possible. |
